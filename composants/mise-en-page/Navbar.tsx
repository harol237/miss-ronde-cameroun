          ...
          <div className="flex gap-1">
            {['fr', 'en', 'es'].map((l) => (
              <Link
                key={l}
                href={'/' + l}
                className={`text-[10px] font-medium tracking-[0.1em] uppercase px-2 py-1 rounded transition-all duration-200 ${locale === l ? 'text-[#C9A84C] border border-[#C9A84C]/50' : 'text-gray-100 hover:text-white'}`}
              >
                {l}
              </Link>
            ))}
      ...
      <div className={`lg:hidden bg-black/98 transition-all duration-500 overflow-hidden ${menuOuvert ? 'max-h-screen py-6' : 'max-h-0'}`}>
        <div className="px-6 flex flex-col gap-4">
          ...
          <div className="flex gap-2 pt-2">
            {['fr', 'en', 'es'].map((l) => (
              <Link key={l} href={'/' + l} className={`text-[10px] font-medium uppercase px-3 py-1.5 border transition-all ${locale === l ? 'text-[#C9A84C] border-[#C9A84C]/50' : 'text-gray-100 border-white/10'}`}>{l}</Link>
            ))}
          </div>
